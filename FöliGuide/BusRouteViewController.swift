...
class BusRouteViewController: UIViewController {
    ...
    func scrollToNextBusStop(animated: Bool) {
        ...
        guard let nextStopIndex = displayStops.index(where: { $0.name == appDelegate.busController.currentUserBus?.nextStop.name }) else {
            return
        }

        self.busStopsTableView.scrollToRow(at: IndexPath(row: nextStopIndex, section: 0), at: .top, animated: animated)
    }
    ...
    func hasDuplicateStops(_ stops: [BusStop]) -> Bool {
        for (index, stop) in stops.enumerated() where index > 0 && stops[index - 1].name == stop.name {
			return true
        }
...
extension BusRouteViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let appDelegate = appDelegate,
            let _ = appDelegate.busController.currentUserBus?.route else {
		...
		let nextStopIndex = displayStops.index(where: { $0.name == appDelegate.busController.currentUserBus?.nextStop.name })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if selectedStop.name == destinationStop {
            ...
            alertController.addAction(UIAlertAction(title: "Remove", style: .destructive, handler: { _ -> Void in
                ...
                for (index, _) in self.displayStops.filter({ $0.name == self.destinationStop }).enumerated() {
            ...
            alertController.addAction(UIAlertAction(title: "Set", style: .default, handler: { _ -> Void in
				...
				if let previousDestinationStop = previousDestinationStop {
                    for (index, stop) in self.displayStops.enumerated() where stop.name == previousDestinationStop {
						if let cell = self.busStopsTableView.cellForRow(at: IndexPath(row: index, section: 0)) as? RouteStopTableViewCell {
							cell.alarmImageView.isHidden = true
						}
                    }
