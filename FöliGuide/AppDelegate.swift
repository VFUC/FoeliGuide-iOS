...
protocol BusUpdateDelegate: class {
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func initialDataLoading() {
        if busStops == nil { //Get bus stop data once, if not retrieved yet
            busController.getBusStops { (stops) -> Void in
                self.busStops = stops
            }
        }
        ...
        self.busController.getBussesOnce({ (busses) -> Void in
    ...
    func startBusDataLoop() {
        ...
        self.busController.getBussesInLoop(intervalInSeconds: refreshInterval, completionHandler: { (busses) -> Void in
            guard let busses = busses else { // failure getting busses
                return
            }
            ...
            for bus in busses where bus.vehicleRef == self.busController.currentUserBus?.vehicleRef {
                ...
                if let route = updatedBus.route {
                    ...
                    var oldBusStopRouteIndex: Int?
                    var newBusStopRouteIndex: Int?
